...
**Цепочка «техники стрелки» (`preceding`, с 2026-08-13).** Необязательный список предыдущих звеньев, от первого к последнему, до 8 штук. iOS шлёт его, когда мысль — не первая в цепочке «И что с того? Что в этом ужасного?». Блок про цепочку добавляется в **user**-промпт (`build_user_prompt`), а не в system: системный вылизан под одиночную мысль и содержит «не придумывай обстоятельств, которых нет во вводе», а строка про цепочки висела бы на каждом запросе, включая те, где цепочки нет. Блок явно говорит разбирать ТОЛЬКО текущую мысль — иначе модель норовит вернуть метки всей цепочки разом.

Без этого поля глубокое звено («значит я никому не нужен») уезжало обрывком: модель не знала, что это ответ на вопрос к предыдущей мысли, и метки выходили беднее. На клиенте предыдущие звенья входят в сигнатуру анализа — правка первого звена переразбирает все нижние, иначе они остались бы с разбором по уже несуществующему контексту.
